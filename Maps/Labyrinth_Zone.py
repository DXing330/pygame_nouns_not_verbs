...
class Labyrinth(Battle_Zone):
    ...
    def decide_possible_passages(self):
        ...
        self.passage_0 = pygame.Rect((self.width - self.width//10)//2, 0, self.width//10, self.width//10)
        self.passage_1 = pygame.Rect(0, self.height - self.width//10, self.width//10, self.width//10)
        self.passage_2 = pygame.Rect(self.width - self.width//10, self.height - self.width//10, self.width//10, self.width//10)
        self.passage_3 = pygame.Rect((self.width - self.width//10)//2, self.height - self.width//10, self.width//10, self.width//10)
        self.passage_4 = pygame.Rect(0, 0, self.width//10, self.width//10)
        self.passage_5 = pygame.Rect(self.width - self.width//10, 0, self.width//10, self.width//10)
    ...
    def check_for_rescue_missions(self):
        ...
        if self.rescue_npc is not None:
            self.rescue_npc_floor = random.randint(0, self.total_floors-1)
            self.rescue_npc_rect.x = random.randint(0, self.width - self.width//20)
            self.rescue_npc_rect.y = random.randint(0, self.height - self.width//20)

    def draw_rescue_npc_location(self):
        if self.floor == self.rescue_npc_floor:
            self.rescue_npc_rect.width = self.width//20
            self.rescue_npc_rect.height = self.width//20
    ...
    def finished_lab(self):
        ...
        for quest in self.party.quests:
            ...
            if quest.requirement == "Save" and quest.location == self.location.name and self.reached_rescue_npc:
                ...
                if not quest.completed and quest.reason != "Story":
                    quest.failed = True
            ...
            for event in pygame.event.get():
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_SPACE:
                        pygame.event.clear()
                        passage = None
                        if self.passage_0.contains(self.player.rect):
                            passage = 0
                        if self.passage_1.contains(self.player.rect):
                            passage = 1
                        if self.passage_2.contains(self.player.rect):
                            passage = 2
                        if self.passage_3.contains(self.player.rect):
                            passage = 3
                        if self.passage_4.contains(self.player.rect):
                            passage = 4
                        if self.passage_5.contains(self.player.rect):
                            passage = 5
                        if passage != None:
